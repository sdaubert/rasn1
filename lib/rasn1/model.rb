  ...
  class Model
    ...
    def initialize_elements(obj, args)
      args.each do |name, value|
        ...
        when Array
          initialize_array(obj[name], value)
        else
    ...
    def initialize_array(obj, value)
      composed = obj.is_a?(Model) ? obj.root : obj
      if composed.of_type.is_a? Model
        value.each { |el| composed << initialize_elements(composed.of_type.class.new, el) }
      else
        value.each { |el| obj << el }
      end
    end
